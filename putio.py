...
class Item(BaseObj):
    ...
    def upload_file(self, **arguments):
        """
        Takes: file
        Returns: True or False depending on the success of uploading process.

        Sends the requests to RPC and returns
        """
        ...
        if fileitem.filename:
            ...
            open('/home/dev/projects/putio/web/uploads/api' + fn, 'wb').write(fileitem.file.read())
            if(os.path.isfile('/home/dev/projects/putio/web/uploads/api' + fn)):
                result = _send(self.api, path="/files", post=args, method="upload")
            else:
                result = "There was a problem in uploading the file."
        else:
            result = "There was a problem in uploading the file."
...
class UrlBucket(object):
    ...
    def fetch(self):
        ...
        Returns None if there is no transfer in progress or active.

        PS: Fetch method doesn't return newly added urls. It returns all the
        active transfers. So you need to check what you just fetched is in the
        transfers list or not.

        Will be improved on v2.
